...
#[no_mangle]
pub unsafe extern "C" fn jieba_with_dict(dict_path: *mut c_char) -> *mut CJieba {
    let cstring_dict_path = std::ffi::CString::from_raw(dict_path);
    let dict = File::open(cstring_dict_path.to_str().expect("字典文件路径错误")).expect("打开字典文件失败");
    let mut dict_buf_reader = BufReader::new(dict);
    let jieba = Jieba::with_dict(&mut dict_buf_reader).expect("加载字典文件失败");
    mem::forget(cstring_dict_path);
    Box::into_raw(Box::new(jieba)) as *mut CJieba
}
...
#[no_mangle]
pub unsafe extern "C" fn jieba_load_dict(j: *mut CJieba, dict_path: *mut c_char) {
    let jieba = j as *mut Jieba;
    let cstring_dict_path = std::ffi::CString::from_raw(dict_path);
    let dict = File::open(cstring_dict_path.to_str().expect("字典文件路径错误")).expect("打开字典文件失败");
    let mut dict_buf_reader = BufReader::new(dict);
    (*jieba).load_dict(&mut dict_buf_reader).expect("加载字典文件失败");
    mem::forget(cstring_dict_path);
}
...
#[no_mangle]
pub unsafe extern "C" fn jieba_add_word(j: *mut CJieba, word: *const c_char, len: usize, freq: isize, tag: *const c_char, tag_len: usize) -> usize {
    let jieba = j as *mut Jieba;
    ...
    let tag: Option<String> = if tag == std::ptr::null() {
        None
    } else {
        let c_str = CFixedStr::from_ptr(tag, tag_len);
        // FIXME: remove allocation
        let s = String::from_utf8_lossy(c_str.as_bytes_full());
        Some(s.into_owned())
    };

    (*jieba).add_word(&s, freq, tag.as_deref())
}
